...
public class AllImageAdapter extends RecyclerView.Adapter<AllImageAdapter.ImageViewHolder> {
    ...
    @Override
    public void onBindViewHolder(@NonNull AllImageAdapter.ImageViewHolder holder, int position) {
        holder.onBind(listImg.get(position).getPath(), position);
    }
    ...
    public class ImageViewHolder extends RecyclerView.ViewHolder {
        ...
        public void onBind(String image_path, int position) {
            Glide.with(context).load(image_path).into(img);

            img.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Intent resultIntent = new Intent();
                    resultIntent.putExtra(KEY_THUMBNAIL, image_path);
